...
async function getArticlesInfo(sourceURL, data) {
    ...
    const collectionLinks = mainPage('.issue-summary .media-body .title')
        .map((i, el) => {
    ...
    const collectionPages = await Promise.all(collectionLinks.map((collectionLink) => fetchData(collectionLink).then((responseData) => cheerio.load(responseData))));
    ...
    for (let collectionPage of collectionPages) {
        ...
        const htmls = await Promise.all(articleLinks.map((articleLink) => fetchData(articleLink).then((responseData) => cheerio.load(responseData))));
